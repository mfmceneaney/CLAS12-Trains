...
public class WriteKinematicsTrain {

    public WriteKinematicsTrain() {}
    ...
    public static void main(String[] args){
        ...
        WriteKinematicsTrain test_wagon = new WriteKinematicsTrain();
        ...
        if (folder.isFile() && folder.getName().contains(".hipo")) {
            ...
            schemaFactory.readFile("/Users/mfm45/trains/lib/kinematics.json"); //NOTE: Need absolute path here.
	...
	public void processDataEvent(Event event, SchemaFactory factory, HipoWriter writer) {
        ...
        Bank kinematics = new Bank(factory.getSchema("REC::Kinematics"),1); //IMPORTANT: DO NOT LEAVE OFF SECOND ARGUMENT: 1 = # of rows 
		event.read(bank);
            ...
            if (pid == 2212) {
                ...
                for (int index2 = 0; index2 < bank.getRows(); index2++) {
                    ...
                    if (pid2 == -211) { 
                        ...
                        if (this.computeCuts(px_e,py_e,pz_e,p2_e,px_pi,py_pi,pz_pi,p2_pi,px_p,py_p,pz_p,p2_p,bank,index_e,index,index2,writer,kinematics,nevent)) {
                            return;
                        }
                    } // if (pid2 == -211)
                } // for ( index2 in bank...)
    ...
    private boolean computeCuts(double px_e, double py_e, double pz_e, double p2_e,
                                                      double px_pi, double py_pi, double pz_pi, double p2_pi,
                                                      double px_p, double py_p, double pz_p, double p2_p, Bank bank, int idxe, int idxp, int idxpi, HipoWriter writer, Bank kinematics, Event event) {
        ...
        if (cuts.get("m") && cuts.get("Q2") && cuts.get("W") && cuts.get("y") && cuts.get("xF") && cuts.get("z")) {
            this.fillKinematics(kinematics,idxe,idxp,idxpi,Q2,nu,W,x,y,z,xF,m);
            // event.reset(); //NOTE: For just writing specific banks.
            event.write(kinematics);
            event.write(bank);
    ...
    public void fillKinematics(Bank kinematics, int idxe, int idxp, int idxpi, double Q2, double nu,
                            double W, double x, double y, double z, double xF, double mass) {
        int rows = kinematics.getRows();
        for(int r = 0; r < rows; r++){
            kinematics.putInt("idxe", r, idxe);
            kinematics.putInt("idxp", r, idxp);
            kinematics.putInt("idxpi", r, idxpi);
            kinematics.putDouble("Q2", r, Q2);
            kinematics.putDouble("nu", r, nu);
            kinematics.putDouble("W", r, W);
            kinematics.putDouble("x", r, x);
            kinematics.putDouble("y", r, y);
            kinematics.putDouble("z", r, z);
            kinematics.putDouble("xF", r, xF);
            kinematics.putDouble("mass", r, mass);
        }
    } // fillKinematics()
